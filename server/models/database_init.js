...
pg.connect(connectionString, function(err, client, done) {
  ...
  client.query('CREATE TABLE "users"( \
                  ...
                  "teammate" INTEGER REFERENCES "users" ("userId") DEFAULT NULL, \
  ...
  // client.query('CREATE OR REPLACE FUNCTION update_wrt_registration_func() RETURNS TRIGGER AS $update_wrt_registration_func$ \
  //               DECLARE \
  //                 testResult INTEGER; \
  //                 oldTeamId INTEGER; \
  //               BEGIN \
  ...
  //                   RETURN NULL; \
  //               END; \
  //               $update_wrt_registration_func$ LANGUAGE plpgsql;',
  //   (err, result) => {
  ...
  // //                     ELSE \
  // //                       SELECT teamId AS oldTeam FROM users WHERE users.userId = NEW.teammate INTO oldTeamId; \
  // //                       IF NOT EXISTS oldTeamId THEN \
  // //                         RAISE NOTICE \'Teammate does not exist\' ; \
  // //                       END IF; \
  // //                       SELECT COUNT(teamMember) FROM teamMembers WHERE teamId = oldTeam INTO testResult ; \
  // //                       IF testResult = 4 THEN \
  // //                         RAISE EXCEPTION \'Full Team --> %\', oldTeamId USING HINT = \'Please choose another team or leave it blank to create a new team\'; \
  // //                       END IF; \
  // //                       INSERT into teamMembers (teamId, teamMember) VALUES (oldTeamId, NEW.userId); \
  // //                       NEW.teamId := oldTeamId; \
  // //                     END IF; \
  // //                     RETURN NEW; \
  // //                   ELSIF (TG_OP = \'DELETE\') THEN \
