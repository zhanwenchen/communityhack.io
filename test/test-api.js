...
describe('API:users', function() {
  ...
  it('should update a SINGLE user on /api/v1/user/:userId PUT', (done) => {
    pg.connect(connectionString, (err, client) => {
      // Handle connection errors
      if(err) {
        done();
      ...
      client.query(
        ...
        RETURNING "userId" \
        ;'
        , ["Zhanwen", "Chen", "zhchen@vassar.edu", 2017]
        , function(err, result) {
          // console.log("result is %j", result);
          // console.log("result.rows is %j", result.rows[0].userId);
          newUserId = result.rows[0].userId;
          chai.request(server)
            ...
            .end(function(err, res){
              ...
              res.body.should.have.property('firstName');
              res.body.should.have.property('lastName');
              res.body.firstName.should.equal('Zhanwen');
              res.body.lastName.should.equal('Chen');
              res.body.year.should.equal(2017);
              done();
          });
      });
    });
